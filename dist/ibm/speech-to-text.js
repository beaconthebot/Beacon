  ...
  function micON() { // When the microphone button is clicked
    if (recording === false) {
      if (records === 0) { // The first time the mic is clicked - inform user
        Api.setWatsonPayload({output: {text: ['Accept the microphone prompt in your browser. I will listen soon.'], ref: 'STT'}}); // Dialog box output to let the user know we're recording
        records++;
      } else {
        ...
        stream.promise()                                // Once all data has been processed...
          .then(function(data) {                        // ...put all of it into a single array
            ...
            if (data.length !== 0) {                    // If data is not empty (the user said something)
              ...
              Api.setWatsonPayload({output: {text: ['Microphone input cancelled. Please press the button to speak to me again']}}); // If the user clicked the microphone button again to cancel current input
            }
          })
          .catch(function(err) { // Catch any errors made during the promise
            ...
            mic.setAttribute('class', 'inactive-mic'); // Reset our microphone button to visually indicate we aren't listening to user anymore
            Api.setWatsonPayload({output: {text: ['Beacon timed out after a few seconds of inactivity. Please press the button to speak to me again.']}});
          });
      })
